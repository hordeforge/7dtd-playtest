...
def acquire_exclusive_lock(
    ...
    *,
    ...
    """
    try:
        playtest_lock.acquire(session, path=path, live_probe=live_probe)
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        ...
        type=Path,
        default=(
            Path(os.environ["PLAYTEST_SUITE_FILE"])
            if os.environ.get("PLAYTEST_SUITE_FILE")
            else None
        ),
        help=(
            "optional declarative suite JSON (env PLAYTEST_SUITE_FILE). "
            "When set, its target/host knobs overlay CLI defaults"
        ),
    )
    ap.add_argument(
        "--server",
    ...
    # Declarative suite overlay (optional): topology knobs from suites/*.json.
    ...
    if args.suite_file is not None:
        ...
        # First token of --suite may match a built-in declarative doc.
    ...
    if suite_doc is not None and suite_doc.host.fixtures and not args.no_fixtures:
        # Declarative host.fixtures arms fixture handlers unless explicitly off.
        args.host_fixtures = True
    args._target_plan = target_plan
    args._suite_doc = suite_doc
    ...
    try:
        ...
        try:
            ...
            # claim unheartbeated.
            # With --no-server the dedicated server is intentionally already
            # running (that is what we attach to), so the exclusivity gate
            # only blocks on a stray client, not on a present server.
